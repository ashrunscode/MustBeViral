# 17 — Gap Register (current, post-Run-18)

> Source of truth as of 2026-05-10 after Runs 1-18. The original baseline tables are archived at `codex-audit/_archive/17_GAP_REGISTER_baseline_2026-05-08.md`. Closure rows reference the entry in `codex-audit/FIX_LOG.md` (head + post-Run-XX override footers) where the change landed.
...
| C-3 | Critical | 🟡 **PARTIAL** | Run 6 + Run 9 — Workers AI branch live for `@cf/...` models; Kimi external provider routed via AI Gateway when configured; mock-safe fallback otherwise. Live external activation still requires real account vars + secrets + staging smoke |
...
| H-4 | High | ✅ **CLOSED** | Run 14 — `.github/workflows/validate.yml` no-deploy CI gate added; Run 18 — Git remote configured (`origin` → `https://github.com/ernijsansons/MustBeViral.git`, private), `master` pushed (currently the historic Milestone 8 commit `1864c48`; the dirty Run 1-17 worktree has not yet been committed and remains local) |
...
| CF-MCP-AUTH | Medium | 🟡 **PARTIAL** | Run 17 — Cloudflare API MCP is now authenticated (account `d2897bdebfa128919bd89b265e6a712e`), so D1/KV/R2 read-only discovery is unblocked at the API layer. **Wrangler CLI still reports `Not logged in`** in this shell, so `npm run cf:readiness` still fails. Restoring CLI auth (interactive `wrangler login` or `CLOUDFLARE_API_TOKEN` env) remains a user-side action |
...
Originally 51 gaps + 3 added (CSRF-1, CF-R2-MISSING, CF-MCP-AUTH) = 54 tracked rows; **36 fully or partially resolved** as of Run 18. Production Cloudflare resources live: D1 `mustbeviral-production`, KV `mustbeviral-production-cache`, R2 `mustbeviral-production-media`. Staging Cloudflare resources now also live (Run 18): D1 `mustbeviral-staging`, KV `mustbeviral-staging-cache`, R2 `mustbeviral-staging-media` — all migrated, all wired into `wrangler.jsonc`. Git remote `origin → ernijsansons/MustBeViral` (private) configured; `master` pushed. Local gate green at 12 files / 46 unit tests + 6/6 Playwright; worker bundle 620 KB. Headline remaining: AI-3 external AI provider activation (Kimi mandated, others deferred per user — needs `KIMI_API_KEY` + `AI_GATEWAY_*` secrets), observability dashboards (M-16), Stripe live activation (4 operational gates; Stripe MCP/CLI not present in this shell), Wrangler CLI auth restoration (CF-MCP-AUTH partial — local-shell-only), production redeploy of the dirty worktree, and committing+pushing the Run 1-17 changes to GitHub.
