...
import { runAuthorityRead } from './authority-transaction.mjs';
...
function preflight() {
  ...
  scanCleanroom();
  if (process.exitCode && process.exitCode !== before) return;
...
try {
  runAuthorityRead({
    root: repoRoot,
    lockContext: { operation: 'preflight' },
    read: preflight,
  });
} catch (error) {
  console.error(
    `${error instanceof Error ? error.message : String(error)}. Run pnpm agent:recover if the transition was interrupted.`,
  );
  process.exitCode = 1;
}
