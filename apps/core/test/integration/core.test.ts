...
import { ApiErrorSchema, HealthResponseSchema } from '@mustbeviral/contracts';

describe('Core Worker HTTP contract', () => {
  it('returns deterministic liveness without durable dependency probes', async () => {
    const response = await exports.default.fetch('http://core.test/health', {
  ...
  it('returns a safe not-found envelope with request ID parity', async () => {
    const response = await exports.default.fetch('http://core.test/missing');
    const body = await response.json();
    const requestId = response.headers.get('x-request-id');

    expect(response.status).toBe(404);
    expect(requestId).toBeTruthy();
    expect(ApiErrorSchema.parse(body)).toMatchObject({
      code: 'NOT_FOUND',
