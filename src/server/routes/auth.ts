...
authRoutes.post(
	"/signup",
	ipRateLimit({ scope: "auth-signup", max: 60, windowSeconds: 60 }),
	async (c) => {
		...
		if (existing) {
			return c.json(
				errorEnvelope("EMAIL_IN_USE", "A user with this email already exists.", requestId),
				409,
			);
		}
		...
		return c.json(
			successEnvelope(
				{ user: { id: userId, email, name: parsed.data.name ?? null, role: "user" } },
				requestId,
			),
			201,
		);
...
authRoutes.post(
	"/login",
	ipRateLimit({ scope: "auth-login", max: 30, windowSeconds: 60 }),
	async (c) => {
		const requestId = c.get("requestId");
		...
		if (!user) {
			return c.json(
				errorEnvelope("INVALID_CREDENTIALS", "Invalid email or password.", requestId),
				401,
			);
		}
		if (user.locked_until && new Date(user.locked_until).getTime() > Date.now()) {
			return c.json(
				errorEnvelope("ACCOUNT_LOCKED", "Account is temporarily locked.", requestId),
				423,
			);
		}
