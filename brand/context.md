...
- 2026-09-16 **Dimmed canvas nodes fail WCAG AA** — `.node-dim` renders enabled, focusable node buttons at opacity .5, measuring 2.14:1 for mono text and 3.15:1 for titles. No registered authority document fixes that look, and the Experience Contract requires AA. How should an inactive node read as inactive while its text still meets 4.5:1? — **owner** — open
- 2026-09-16 **API-key scope checkboxes below the 44 px touch target** — the eleven scope labels sit 8 px apart, so their hit areas reach 26.75 px (meets WCAG 2.5.8 at 24 px, not the Experience Contract's 44 px). Meeting 44 px needs more vertical spacing in that list — **owner** — open
