...
- 2026-09-16 **`.superdesign/design-system.md` is stale on accessible colour** — after the WCAG AA fix it still gives `.50` ink for small captions, `signal #3182d4` as the white-text primary fill, and status tokens as label colours (lines 26, 30–38, 83, 88). `packages/ui/src/tokens.ts`, `brand/BRAND.md` and `docs/ux/EXPERIENCE_CONTRACT.md` are current; the file is outside WP-P3-009's allowed paths — **owner + engineering** — open
