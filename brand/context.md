...
| Design tooling          | Any mockup tool may produce frames; no design-generation CLI is a repository dependency                                                                                                                                                                                                                        | Tools are optional and never an authority. Accepted UX authority: `docs/ux/EXPERIENCE_CONTRACT.md`; design brief: `brand/BRAND.md`.                                                                                                                                                              |
...
- 2026-09-16 **`.superdesign/design-system.md` was stale on accessible colour** — it still prescribed `.50` ink for small captions, `signal #3182d4` as the white-text primary fill and status tokens as label colours. Aligned with `packages/ui/src/tokens.ts`, `brand/BRAND.md` and `docs/ux/EXPERIENCE_CONTRACT.md` on 2026-09-16 — **engineering** — resolved
- 2026-09-16 **Dimmed canvas nodes fail WCAG AA** — `.node-dim` renders enabled, focusable node buttons at opacity .5, measuring 2.14:1 for mono text and 3.15:1 for titles. No registered authority document fixes that look, and the Experience Contract requires AA. How should an inactive node read as inactive while its text still meets 4.5:1? — **owner** — resolved 2026-09-16 under the owner's delegated design call: opacity is gone. An inactive node keeps opacity 1 and recedes through non-text chrome only — a dashed `--ink-muted` border as the non-colour cue, 3.91:1 against the canvas at rest and with keyboard focus and 5.59:1 in `--ink` on hover, the `--paper-2` canvas-field fill instead of the raised white card, a transparent status chip and an `--ink-muted` decorative status icon — and its title steps down from `--ink-strong` to `--ink`. On that fill the title and mono labels measure 5.59:1 and the status labels 4.84–5.59:1. The rule is recorded in the accessibility contract of `docs/ux/EXPERIENCE_CONTRACT.md`
- 2026-09-16 **API-key scope checkboxes below the 44 px touch target** — the eleven scope labels sit 8 px apart, so their hit areas reach 26.75 px (meets WCAG 2.5.8 at 24 px, not the Experience Contract's 44 px). Meeting 44 px needs more vertical spacing in that list — **owner** — resolved 2026-09-16: each scope row is at least 24 px tall with 24 px between rows, a 48 px pitch on the 4 px grid, and each label's transparent hit area grows 12 px each way to meet its neighbour at the midpoint, so every checkbox has a 48 px target that overlaps no other. Where the dialog is wide enough the scopes take two columns of at least 160 px with 16 px between them, so from 768 px up the dialog measures 567.63 px and does not scroll. On a phone one column is the only fit and the scope list scrolls, while the Issue key and Cancel row sticks to the bottom of the dialog on the card background with a hairline above it, so Issue key stays in view without scrolling on 320–414 px phones at 100% zoom; 96 px of scroll padding keeps a focused control clear of that 81 px row. Below a 300 px-wide or 320 px-tall viewport (a 320 px phone at 125% zoom or more, a 375 px or 414 px phone at 150% or more) the two buttons would wrap into a 131 px row that covers focused fields, so the row is not sticky there and Issue key is reached by scrolling
