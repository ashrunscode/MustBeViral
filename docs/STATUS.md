...
| Release target | `P0` |
| Pending decisions | DECISION_NO_LEGACY_OR_DNS_MUTATION_WORDING: acceptance no-legacy-or-dns-mutation was recorded passed on 2026-09-02 against an unqualified no-DNS-record-change claim. Three Resend delivery records (resend._domainkey TXT, send.mustbeviral.com TXT, send.mustbeviral.com MX) were added to mustbeviral.com to enable owner email delivery; the zone was read first-hand on 2026-09-03 and holds 10 records total. No A record, CNAME, route, or custom domain changed, api.mustbeviral.com has no record of any type, and legacy V1 traffic is unaffected, so the protection the criterion exists for is intact. Re-expressing an accepted criterion is not a packet-level decision; an owner ruling is required. The related customer-row clause is already ruled: the owner classified the single new Auth row as the approved owner identity, not a customer. |
