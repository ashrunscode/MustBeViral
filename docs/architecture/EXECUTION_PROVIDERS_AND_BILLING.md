...
- A verified Stripe webhook settles before Core records its `stripe_webhook_events` receipt. A settlement failure records no receipt and returns 5xx, so Stripe retries. Settlement is idempotent on the Stripe event id across workspaces: a wallet credit replayed without an explicit workspace keeps the workspace it first credited, and any other replay that resolves a different workspace fails with `STRIPE_EVENT_WORKSPACE_MISMATCH` and applies nothing. That failure, and an event whose workspace cannot be resolved, returns 5xx on every Stripe retry until an operator resolves it.
