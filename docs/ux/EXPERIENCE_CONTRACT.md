...
- An enabled, focusable control never shows an inactive or de-emphasised state through opacity, because opacity pulls its text below AA (a canvas node at opacity .5 measured 2.14:1 for mono text and 3.15:1 for its title). It recedes through non-text chrome only, keeps a non-colour cue, and every text keeps an AA ink. An inactive canvas node, outside the selected node's lineage, takes a dashed `--line` border as that cue, the canvas-field fill `--paper-2` instead of the raised card, a transparent status chip and an `--ink-muted` decorative status icon; its title steps down from `--ink-strong` to `--ink`, and on that fill its title and mono labels measure 5.59:1 and its status labels 4.84–5.59:1.
